    ...
    <div className="space-y-6" id="audit-logs-tab">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Immutable tracker registry monitor recording all administrative actions, agent registrations, deactivations, and conflict resolutions.</p>
        </div>

        <button 
          onClick={fetchLogs}
          className="p-2 bg-white hover:bg-slate-50 border border-slate-200 text-slate-600 rounded-lg shadow-sm hover:shadow transition-all font-semibold text-xs inline-flex items-center gap-1.5 cursor-pointer"
        >
      ...
      {loading ? (
        <div className="bg-white rounded-xl border border-slate-100 p-16 text-center shadow-sm">
          ...
          <p className="text-sm text-slate-400">Querying secure compliance vault...</p>
        </div>
      ) : filteredLogs.length === 0 ? (
        <div className="bg-white rounded-xl border border-slate-100 p-16 text-center shadow-sm text-slate-450">
          <p className="text-sm text-slate-500">No matching security entries found matching filters.</p>
        </div>
      ) : (
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden" id="audit-ledger-table-container">
                ...
                {filteredLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div>
                        <div className="font-semibold text-slate-900 leading-snug">{log.userName}</div>
                    ...
                    <td className="px-6 py-4 min-w-[300px]">
                      {log.newValue ? (
                        <div className="space-y-1.5 max-w-sm">
                          ...
                          <div className="text-xs text-teal-850 bg-teal-50/50 p-2 border border-teal-100/50 rounded break-all max-h-24 overflow-y-auto font-mono">
                            <span className="font-bold uppercase text-[9px] mr-1 bg-teal-200/50 px-1 py-0.5 rounded">New:</span> 
                            {log.newValue}
                          </div>
                        </div>
                      ) : (
                        <span className="text-xs text-slate-450 italic">None logged.</span>
                      )}
                    </td>
                  </tr>
                ))}
